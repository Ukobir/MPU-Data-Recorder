...
#define BUZZER_A 21
#define BUZZER_B 10
const uint8_t LED_RGB[3] = {11, 12, 13};

static char filename[20] = "mpu6050_data.txt";
...
static uint8_t montar = 0;
static bool escreve = false;
...
int main()
{
    ...
    initBotao(botaoSW);
    gpio_set_irq_enabled(botaoSW, GPIO_IRQ_EDGE_FALL, true);
    // Fim
    // Inicializa os LEDS
    initLEDs(LED_RGB);
    ...
    ssd1306_rect(&ssd, 0, 0, 126, 62, true, false);
    ssd1306_draw_string(&ssd, "Aperte B", 33, 17);
    ssd1306_draw_string(&ssd, "Para Montar o", 10, 30);
    ssd1306_draw_string(&ssd, "o cartao SD", 10, 40);
    ssd1306_send_data(&ssd);
    ...
    while (true)
    {
        ...
        if (montar == 1 & !onetime)
        {
            ...
            ssd1306_send_data(&ssd);
            ssd1306_rect(&ssd, 0, 0, 126, 62, true, false);
            ssd1306_draw_string(&ssd, "Aperte A", 33, 7);
            ssd1306_draw_string(&ssd, "Para Gravar", 10, 17);
            ssd1306_draw_string(&ssd, "Aperte B", 33, 35);
            ssd1306_draw_string(&ssd, "Desmontar SD", 10, 45);
            ssd1306_send_data(&ssd);
            gpio_put(LED_RGB[0], 1);
        ...
        else if (montar == 2)
        {
            ...
            ssd1306_send_data(&ssd);
            ssd1306_rect(&ssd, 0, 0, 126, 62, true, false);
            ssd1306_draw_string(&ssd, "Cartao SD", 33, 7);
            ssd1306_draw_string(&ssd, "DESMONTADO", 10, 17);
            ssd1306_draw_string(&ssd, "Aperte B", 33, 35);
            ssd1306_draw_string(&ssd, "Montar SD", 10, 45);
            ssd1306_send_data(&ssd);
            gpio_put(LED_RGB[0], 1);
            gpio_put(LED_RGB[2], 1);
            gpio_put(LED_RGB[1], 0);
        ...
        if (escreve & montar == 1)
        {
            ...
            ssd1306_send_data(&ssd);
            ssd1306_rect(&ssd, 0, 0, 126, 62, true, false);
            ssd1306_draw_string(&ssd, "Gravacao em", 10, 7);
            ssd1306_draw_string(&ssd, "ANDAMENTO!", 33, 17);
            ssd1306_draw_string(&ssd, "Aperte A", 33, 35);
            ssd1306_draw_string(&ssd, "Para Parar", 10, 45);
            ssd1306_send_data(&ssd);
            ...
            while (escreve & montar == 1)
            {
                gpio_put(LED_RGB[0], 0);
                gpio_put(LED_RGB[2], 0);
                gpio_put(LED_RGB[1], 1);
                mpu6050_read_raw(aceleracao, gyro);
                sprintf(buffer, "%d,%d,%d,%d,%d,%d,%d\n", i + 1, aceleracao[0], aceleracao[1], aceleracao[2], gyro[0], gyro[1], gyro[2]);
                res = f_write(&file, buffer, strlen(buffer), &bw);
                if (res != FR_OK)
                {
                    gpio_put(LED_RGB[0], 0);
            ...
            f_close(&file);
            printf("\nDados do sensor MPU6050 salvos no arquivo %s.\n\n", filename);
            beep(16e3);
            sleep_ms(400);
            semSom();
            sleep_ms(200);
            ...
            ssd1306_send_data(&ssd);
            ssd1306_rect(&ssd, 0, 0, 126, 62, true, false);
            ssd1306_draw_string(&ssd, "Escrita", 33, 7);
            ssd1306_draw_string(&ssd, "Concluida!", 10, 17);
            ssd1306_draw_string(&ssd, "Aperte B", 33, 35);
            ssd1306_draw_string(&ssd, "Desmontar SD", 10, 45);
            ssd1306_send_data(&ssd);
        }

        //Demais funções do programa no monitor serial
